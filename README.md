...
**A grow box that keeps a plant alive without anyone tending it, built inside a scavenged PC
tower.** Temperature, humidity and light are measured continuously; lights, fans and water pumps
are switched on a daily cycle; the whole thing is monitored and overridden from a phone on the
local network. The plant is hydroponic, so there is no soil to water and nothing to remember.
...
![The PFC in 2026, full of growth under the grow lights, with one flower in bloom](docs/pfc-2026.jpg)

*The same box in **2026**, five years after the defence, still running its daily cycle. The
enclosure is now full: stems reaching the LED bar, one flower in bloom, and a mat of moss and
undergrowth covering the water tank at the base. What it grows is no longer basil, and what the
Raspberry Pi does is no longer only this, see [Five years later](#five-years-later).*
...
you control instead of ones you inherit, with temperature, humidity and photoperiod all set
...
same dead computer, one is the original case fan and the other came off its power supply. What we
...
it with a tank that needs topping up every few weeks.
...
The functional analysis behind it (`FP1`/`FP2` principal functions, `FC1` to `FC6` constraints) is
...
flowchart LR
    ...
    MAIL["SMTP to e-mail<br/>high-temperature alert"]
...
instinct, since we had built exactly that the year before for the aquarium, and the client pushed
...
it (see [Status](#status-the-flow-here-is-a-2021-snapshot)).
...
Per-device wiring diagrams: [sensors](docs/wiring-sensors.png) ·
[actuators](docs/wiring-actuators.png), both also in [the report](report/PFC_Final_Report.pdf).
...
Requires real hardware, since the flow talks to GPIO pins and 1-Wire devices and does not do
...
Airflow is aimed at the **leaves, not the roots**, because ventilating the roots fights the
hydroponics by accelerating evaporation from the tank.
...
Our first idea was a potentiometer, dimming the lights to near-zero instead of switching them
off, leaving the client's hardware intact. We rejected it on two counts: we could not establish
what even a little light during the dark phase does to the plant, and it keeps the lamps powered
...
sunlight, which is not recommended. The proposed fix, tinted plexiglass that is dimmer inside
but still transparent enough to inspect the plant, was never fitted.
...
circulating water oxygenates the tank well enough, decided it does, and kept the mistake.
...
The request was right, since soldered joints survive transport and jumper wires do not, but it
arrived after the money was spent, because *"breadboard"* had been mentioned in a meeting instead
of written into an agreed scope.
...
|---|---|---|
| 1 | Hydroponics running, lights not yet working. Leaves lost strength and flattened, too much water and no light | Germinating |
...
*Closed, in a dark room. The only thing visible from outside is the window we sawed into the
front door and the grow lights behind it, which is the whole argument for cutting the window.*
...
## Status, the flow here is a 2021 snapshot

The PFC ran, grew basil and was tested by real users in 2021. It is *still running*, which is
[the last section](#five-years-later). But what is committed here is the flow as it stood on the
SD card when the project closed, and that snapshot is not a deployable product:
...
- **Charts keep 10 minutes**, not the 20 hours the report describes, since the flow was retuned
  after the text was written. The Grafana long-term history that was supposed to replace them was
  never built.
...
is where the project was heading and because the report argues for it in detail. Read it as the
2021 wish list: one item on it did get built, five years later, and it is not the one we would
have bet on.
...
earning it back with a NAS, wifi router or browser interface, and pairing it with a solar panel,
on the argument that the Pi's demand and a panel's output peak at the same time of day.
...
**In 2026 the PFC still runs, every day.** That is the part of this project I did not expect to
be able to write. It has been in service continuously since the defence, it has grown things the
report never considered, and it has quietly become a different machine than the one we submitted.

### What actually grows well

Basil was the plant we designed around, and basil is not the best thing to put in this box.

- **Chives** are the strongest result to date, the closest thing to a plant that simply works in
  here.
- **Mint** is clearly better than basil: faster, more forgiving, harder to kill.
- **Moss** on the surface of the water tank was an aesthetic experiment rather than a crop, and it
  worked. It spreads into a small lawn over the water, and it looks good enough to justify the
  window.

The pattern is that the plants that thrive are the aggressive, high-turnover herbs, not the ones
with a delicate growth stage to protect. We optimised the cycle for basil's photoperiod; the box
turns out to reward vigour more than tuning.

### What five years of runtime taught that three weeks could not

**Sensors die early or not at all.** A lot of the sensors failed in roughly the first two months.
Every sensor that survived that window is still fully functional today. That is textbook infant
mortality, and it has a practical consequence the report gets wrong by omission: the honest
reliability plan for a box like this is not periodic replacement, it is *burning the batch in* and
then leaving it alone. The five-DHT11 minimum order we treated as an accident of purchasing turned
out to be the right spares count for exactly this reason.

**The pumps were never useful.** Not underused, useless. Reducing them to five minutes every ten
hours was already an admission; the truthful conclusion is that a small hydroponic tank with roots
in it does not need mechanical circulation at all. This retires the whole
[air-pump-versus-water-pump episode](#a-water-pump-where-an-air-pump-should-be): we bought the
wrong part, kept it, justified it, and it did not matter either way.

**The tank is too small.** The report's user guide says *"add some water every month"*, which is
what we genuinely expected. In practice it needs topping up **every two to three weeks**, because
the plants drink it down faster than we sized for. Not a dramatic miss, but it means tank volume,
not light, not temperature, not humidity, is what actually bounds how long the box runs
unattended. Everything the project automated was a variable we could sense cheaply. The one
variable that ended up governing autonomy is the one we sized by eye and never instrumented.

### It became the IoT hub after all

The most-discussed idea in [Perspective](#perspective-designed-not-built) was that a Pi 4B driving
five relays is over-specified, and that the spare capacity should be earned back. That is the item
that got built, not by plan, but because the machine was already on 24/7 in a useful place:

- **NAS.** Hard drives attached to the Pi, serving storage on the home network.
- **Network-wide ad filtering.** **AdGuard Home**, sitting alongside the home router and filtering
  ads at the DNS level for every device in the flat.
- **Home automation.** Lights and temperature across the flat, not just inside the grow box, with
  the PFC's own cycle now one automation among several.

It is not a well-designed IoT hub. It grew by accretion, one service at a time, and no one would
architect it this way from scratch. But it is a genuinely useful object that I still add features
to now and then, and that is a stranger and better outcome than a finished product would have
been. The grow box was the brief. The thing that survived is a box that grows herbs, stores my
files, cleans my network, and remains the most convenient place I own to try an IoT idea on real
hardware.

Which is the honest closing note for a student project: the parts we engineered hardest, the
pump cycle, the sensor redundancy, the photoperiod tuned for basil, are the parts time argued
with. What lasted was the decision to put a real computer in the box and leave it running.

## Credits

**Julien Rosé · Achille Bayart · Nicolas Rigaudy**, 5th year, IoT major, ESME Sudria.

Client: **Hear & Know**, with Jean-Philippe Lelievre, Hubert Thiriar and Mohammad Reza Zohrabi.
